--+==============================================================================================+
--| Program				: nightlyVersiRentDBMaint												 |
...
--|	Revision 8/23/2016																			 |
--|																								 |
--|		+ Added option to enable Ad-hoc query optimization     								     |
--|																								 |
--|																								 |
--+==============================================================================================+
--================================================================
...
--================================================================
/*       Allow query optimizer to store ad-hoc plans in cache   */
--================================================================
sp_configure 'show advanced options', 1
GO
RECONFIGURE;
...
--		agreemnt table
--=================================================
IF EXISTS (SELECT Name FROM sysindexes WHERE Name='ix_Agreemnt_STID_AStatID_includes')
	DROP INDEX Agreemnt.ix_Agreemnt_STID_AStatID_includes;

IF NOT EXISTS 
	(SELECT Name FROM sysindexes WHERE Name='ix_Agreemnt_CustID_AStatID_AgreeID_DueDate_STID')
		CREATE INDEX [ix_Agreemnt_CustID_AStatID_AgreeID_DueDate_STID] ON [VersiRentDB].[dbo].[Agreemnt] ([CustID],[AStatID],[AgreeID],[DueDate],[STID]) WITH (FILLFACTOR=100, ONLINE=OFF, SORT_IN_TEMPDB=OFF);
...
IF NOT EXISTS 
	(SELECT Name FROM sysindexes WHERE Name='ix_Invntry_STID_InvStatID_InvServStatID_includes')		
		CREATE INDEX [ix_Invntry_STID_InvStatID_InvServStatID_includes] ON [dbo].[Invntry] ([STID],[InvStatID],[InvServStatID]) INCLUDE ([InvID],[StockNumber],[ModID],[Serial],[InvCondID],[DateOfStock],[FirstOut],[TotalIncome],[TimesRented],[DaysRented]) WITH (FILLFACTOR=100, ONLINE=OFF, SORT_IN_TEMPDB=OFF);
...
IF NOT EXISTS 
	(SELECT Name FROM sysindexes WHERE Name = 'ix_PeoplePhones_IsSMS_Deleted_includes')			
			CREATE INDEX [ix_PeoplePhones_IsSMS_Deleted_includes] ON [dbo].[PeoplePhones] ([IsSMS],[Deleted]) INCLUDE ([PeopleID],[PhoneNumber]) WITH (FILLFACTOR=100,ONLINE=OFF,SORT_IN_TEMPDB=OFF);
...
IF NOT EXISTS 
	(SELECT Name FROM sysindexes WHERE Name = 'ix_SMSMessageDetails_SMSStatusID_includes')
		CREATE INDEX [ix_SMSMessageDetails_SMSStatusID_includes] ON [dbo].[SMSMessageDetails] ([SMSStatusID]) INCLUDE ([SMSMessageDetailID],[EmployeeID],[MessageBody]) WITH (FILLFACTOR=100,ONLINE=OFF,SORT_IN_TEMPDB=OFF)
...
IF NOT EXISTS
	(SELECT Name FROM sysindexes WHERE Name = 'ix_StoreStatistics_Modified_includes')
		CREATE NONCLUSTERED INDEX [ix_StoreStatistics_Modified_includes] ON [dbo].[StoreStatistics] ([Modified]) INCLUDE ([STID],[StoreStatID],[SSDate],[SSTypeID],[Value],[LastModDateTime]) WITH (FILLFACTOR=100,ONLINE=OFF,SORT_IN_TEMPDB=OFF);
...
IF EXISTS 
	(SELECT Name from sysindexes WHERE Name='ix_DocumentRoles_DocumentUID')
		DROP INDEX DocumentRoles.ix_DocumentRoles_DocumentUID;

IF EXISTS 
	(SELECT Name from sysindexes WHERE Name='ix_DocumentRoles_DocumentUID_RoleUID')
		DROP INDEX DocumentRoles.ix_DocumentRoles_DocumentUID_RoleUID;

IF NOT EXISTS 
	(SELECT Name from sysindexes WHERE Name='ix_DocumentRoles_DocumentUID_RoleUID')
		CREATE INDEX [ix_DocumentRoles_DocumentUID_RoleUID] ON [dbo].[DocumentRoles] ([DocumentUID],[RoleUID]) WITH (FILLFACTOR=100, ONLINE=OFF, SORT_IN_TEMPDB=OFF);
...
--=============================================
--TRUNCATEONLY WILL NOT BREAK THE LSN CHAIN
USE VersiRentDB
GO
DBCC SHRINKFILE(VersiRentDBLog, TRUNCATEONLY);
--=============================================
